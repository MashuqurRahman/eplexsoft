...
def products_details_page_view(request, pk):
    ...
    try:
        sub_sub_category_obj = admin_dashboard_models.SubSubCategories.objects.get(id=product.sub_sub_categories.id)
        similar_product_obj = admin_dashboard_models.Product.objects.filter(sub_sub_categories_id=sub_sub_category_obj.id).prefetch_related('product_varient').exclude(id=pk)
