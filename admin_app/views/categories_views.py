    ...
    if request.user.is_superuser or request.user.role == 'central_admin':
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                try:
                    category = form.save()
                    create_default_sub = form.cleaned_data.get('create_default_sub')
                    name = form.cleaned_data.get('name')
                    description = form.cleaned_data.get('description')

                    if create_default_sub:
                        # SubCategory create
                        sub_cat = admin_dashboard_models.SubCategories.objects.create(
                            categories=category,
                            sub_cat_name=name,
                        ...
                        admin_dashboard_models.SubSubCategories.objects.create(
                            sub_categories=sub_cat,
                            sub_sub_cat_name=name
                        )
