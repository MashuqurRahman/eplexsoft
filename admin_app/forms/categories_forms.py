...
class CategoriesForm(forms.ModelForm):
    create_default_sub = forms.BooleanField(
        required=False,
        label="Create default Sub & Sub-Sub Category"
    )
...
class SubCategoriesForm(forms.ModelForm):
    ...
    def clean(self):
        cleaned_data = super().clean()
        sub_cat_name = cleaned_data.get('name')
        if admin_dashboard_models.Categories.objects.filter(name__iexact=sub_cat_name).exists():
